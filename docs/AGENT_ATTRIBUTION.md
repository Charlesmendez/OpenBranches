...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Codex           | One connection supplies bounded app-server task history, recent local session activity, and shared-daemon runtime state when available. An optional lifecycle hook adds direct future-turn signals. Current and archived task associations remain available. |
...
The Codex connection also watches recent local rollout metadata. This covers a task that was created in one workspace and later runs a tool in another checkout: the last literal `workdir` supplied to the local execution tool becomes the current folder evidence. A small lexer skips quoted command bodies, comments, and templates, so text that merely mentions a path cannot move the live marker. Turn completion clears the marker. The parser discards commands, prompts, responses, tool input/output, instructions, and file names immediately.
...
Claude Code and Cursor live activity starts disabled. Codex local-session activity is included when Codex is connected; its lifecycle hook remains optional. Enabling a hook in Settings adds one OpenBranches-owned command handler to that tool's user hook file and writes a small reporter script beside it. Codex uses `~/.codex/hooks.json`, Claude uses `~/.claude/settings.json`, and Cursor uses `~/.cursor/hooks.json`. Existing settings and unrelated hooks are preserved. Repeated setup is idempotent, files are replaced atomically, symbolic-link targets are refused, and disconnect removes only the exact OpenBranches handlers and owned script. The app repairs an already approved hook when its private endpoint changes between launches. Codex independently requires the user to review and trust a new or changed hook before it runs; OpenBranches cannot approve that trust decision.
...
Codex activity checks up to 20,000 session-directory entries and follows at most 200 recently modified rollout files. It reads at most the latest 512 KiB once, then only appended bytes, without following file symlinks. A recursive Mac file watcher normally finds changes; a bounded 60-second reconciliation repairs missed events. Known active files are also polled for size changes every five seconds while the app window is active. Those short checks consume only changed file tails; the heavier shared-daemon inspection remains on the normal one-minute refresh. A malformed, oversized, or future-dated record cannot create live activity.
