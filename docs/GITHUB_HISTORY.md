...
The implemented reader was also checked against the public OpenBranches repository: it listed three branches, reused identical integration tips, and correctly classified the feature commit as absent from the integration target. Native service refresh and the new inspector still need a combined packaged-app check. PR metadata now survives remote-branch deletion independently of branch rows, and open work has its own bounded listing. Exact closed-PR branch associations survive deletion without attaching to newer local commits. Targeted older closed-PR lookup remains separate work; see [collaboration evidence](COLLABORATION.md).

Reference: [GitHub REST comparison endpoint](https://docs.github.com/en/rest/commits/commits#compare-two-commits).
