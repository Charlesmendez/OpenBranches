...
Public GitHub reading is available in Settings without sign-in. It only reads remotes of repositories you selected; GitHub's unauthenticated rate limits apply. Private repositories require two GitHub steps. Device sign-in authorizes the user's identity. The OpenBranches Desktop GitHub App must then be installed separately on each personal account or organization whose repositories should be visible; the user chooses all repositories or selected repositories. After sign-in, Settings groups the selected repositories by owner and compares them with the GitHub App installations accessible to the signed-in user. Each owner is labeled **All repositories**, **Selected repositories**, or **Not connected**, with the relevant GitHub setup action. OpenBranches checks this installation list with one bounded conditional request, rechecks when the app regains focus, and refreshes newly available GitHub evidence when the installation scope changes.
...
The local index is checked once per minute while the app window is active, with a 60-second pagination window, 15-second request timeouts, at most 5,000 current and 5,000 archived entries, and bounded protocol messages. Background inspection pauses when the window is hidden or minimized, or is unfocused while the Mac is on battery, then refreshes when it becomes active again. An explicitly approved team share keeps only the local sources needed for that background publication running. The UI labels incomplete results. Initial prompts, conversation bodies, rollout paths, and runtime status are discarded. Origin URLs are reduced to GitHub repository identity, removing credentials and query strings. Only metadata associated with selected projects is retained in OpenBranches' local database.
...
Saved task metadata is historical evidence, not proof that Codex is currently running or owns the branch now. The connection separately watches recent structural events in Codex's local rollout logs and, when available, reads shared-daemon runtime status. A recent tool call supplies its exact literal working directory without retaining its command, prompt, response, or tool input. Only a fresh checkout match can glow. The optional lifecycle hook below adds direct future-turn signals. Unpublished cloud work and other computers are outside this local connection.
...
Disconnect stops inspection and clears OpenBranches' task cache and in-memory account status; it does not sign the user out of Codex or change their conversations. Automatic scheduling, model-generated cleanup actions inside OpenBranches, and edit-mode handoffs remain post-release experiments. The first release uses an explicit read-only handoff after the user reviews its evidence and prompt. The separate advisor budget and evidence-preparation modules are not connected to a model runner. See [advisor research](ADVISOR.md).
...
Settings offers optional **Codex**, **Claude Code**, and **Cursor** lifecycle hooks. Codex already receives automatic local-session activity from its main connection; its hook adds another direct source. Claude Code and Cursor need their hooks for live state. Codex requires the user to review and trust the exact hook before it can run. A fresh signal lights only the exact currently checked-out branch whose worktree folder and head match the latest local Git scan. Stop and response-complete signals remove the working state immediately; any missing follow-up expires after 90 seconds. Codex and Claude can report an explicit waiting-for-input state. Cursor's reported model ID is shown separately, including a Grok icon when Cursor explicitly reports a Grok model.
