...
The open queue is ordered for action: fresh reported check failures first, then explicit review requests, then drafts with no PR update for at least 14 days, followed by the remaining open work. Compact reason chips repeat that evidence on each affected PR, so the ordering is visible rather than hidden. Summary counts always describe the current person, project, tool, and search scope. The people roster remains stable while moving between PR queues so a lead can follow one person across open work and history. This is triage order only; OpenBranches does not infer whether GitHub will allow a merge.
