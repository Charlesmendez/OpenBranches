...
Provider adapters return normalized project, task, and activity evidence with source, checked time, confidence, and optional model. Codex saved-task metadata is one adapter. Claude Code now has an opt-in local metadata reader with bounded fixture and read-only live checks; broader compatibility and native connection verification remain. See [implemented attribution and limits](AGENT_ATTRIBUTION.md). Grok and other models may run inside another tool; support comes from that tool's adapter or a documented OpenBranches event protocol. Do not infer a provider from a branch prefix, a configured model, the presence of an instruction file, or ordinary commit author text. Unknown and historical associations remain visible as such. Use bundled, licensed provider icons with accessible names and text fallbacks; do not load arbitrary icon URLs from repository data.
...
These are implementation stages, not claims of completion. Common tool/model identity, badges, and the Codex/Claude local readers are implemented. People & PRs now displays real connected GitHub authors, requested reviewers and teams, searchable PRs, quiet drafts, and history with preserved navigation; see [implemented collaboration and limits](COLLABORATION.md). PR head check/status results, submitted reviews, and a recent-failure filter are also implemented; see [PR signal evidence and limits](PR_SIGNALS.md). The optional authenticated [team service preview](TEAM_SERVICE.md) implements member/device identity, project permissions, pairing, consent-bound snapshots, revocation, PostgreSQL persistence, and streamed invalidations. Its browser workspace separates opted-in `Live & local` reports from a read-only `Branches & PRs` view. Fresh verified Codex runtime evidence is surfaced above totals, focuses the exact person/project/branch when chosen, sorts active groups and rows first, and expires without a new observation. The GitHub view presents only authorized repositories, exposes branch and PR search plus person/project filters, labels current/partial/failed snapshots, shows PR authors, review requests and checks, and reports exact immutable-SHA membership in develop/dev/main/master without assigning branch ownership. Owner and member views were verified with fictional data. The [Mac companion](TEAM_COMPANION.md) implements pairing, metadata review and approval, background publication, upload status, Stop sharing, and disconnection. Native fictional checks cover automatic commit updates, process restarts, and offline withdrawal recovery. Organization membership synchronization, live Cursor/Grok ingestion, company alerts, real GitHub acceptance, and real two-member/device checks remain unfinished. Existing Git scanning already lists branches independently of the tool that created them; verified tool attribution is separate work.
