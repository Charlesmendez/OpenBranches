...
These are implementation stages, not claims of completion. Common tool/model identity, badges, and the Codex/Claude local readers are implemented. Live Cursor/Grok ingestion, team service, sharing, and company alerts are not implemented yet. Existing Git scanning already lists branches independently of the tool that created them; verified tool attribution is separate work.
