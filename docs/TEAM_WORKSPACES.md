...
These are implementation stages, not claims of completion. Common tool/model identity, badges, and the Codex/Claude local readers are implemented. People & PRs now displays real connected GitHub authors, requested reviewers and teams, searchable PRs, quiet drafts, and history with preserved navigation; see [implemented collaboration and limits](COLLABORATION.md). Live Cursor/Grok ingestion, CI/review-decision evidence, the authenticated team service, sharing, and company alerts remain unfinished. Existing Git scanning already lists branches independently of the tool that created them; verified tool attribution is separate work.
