...
`team-service/dev/preview.ts` is a separate fictional entry point, excluded from the production container. Preview and integration-test scripts share an isolated disposable database helper. The Mac companion integration remains unimplemented. No desktop scan calls the service or uploads metadata. See [protocol, browser preview, deployment, and verification limits](TEAM_SERVICE.md).
