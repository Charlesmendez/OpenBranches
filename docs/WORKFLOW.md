...
Each project toolbar makes live coverage visible without adding another dashboard. On the map, active work appears once in the prominent **Happening now** rail. Every active card includes compact local Git status for the primary development and stable targets, such as **In develop** and **Not in main**. Repositories with more targets show the remaining count; selecting the card opens the complete evidence. In the branch inventory, the toolbar shows the live/waiting count and focuses the highest-priority branch in one click. When no live work is present, the same compact control says which tools are being watched or links directly to live activity setup when coverage is off.
