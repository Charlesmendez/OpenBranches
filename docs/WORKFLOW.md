...
Other tools retain their explicit task/model attribution; their saved metadata is not live presence. PR authors and commit authors are labeled by source and do not establish who is working now. Recent task updates and uncommitted work are distinct activity labels. Human presence and unpublished teammate activity still require an opted-in companion with its own observed evidence.
...
Selecting visible rows supports bulk snooze, dismiss, and restore. These actions only change review choices; they never alter Git. Evidence is expandable within each row. Snoozing lasts seven days, and dismissal remains until the evidence revision changes. The connected Codex label reflects actual provider state and linked task count. These Git findings are not described as AI reviews.
