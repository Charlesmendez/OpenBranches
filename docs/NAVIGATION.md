...
Choosing a project from the sidebar opens the lifecycle, map group, and six-branch page containing its strongest running, waiting, or dirty-worktree signal. It leaves the details drawer closed so the map retains its full width. A live branch has a pulsing blue halo and emphasized verified ancestry or PR lines; a waiting branch uses violet; an uncommitted working copy has a steady amber halo because file changes alone do not prove that a person or agent is currently present. The same labels remain on the branch node after navigation. The inventory adds a compact live/waiting control that focuses the highest-priority current branch on the map. A quiet project shows the connected live sources, while an unconfigured project links directly to live activity settings. A connected Codex source watches recent local task activity automatically; unavailable coverage opens Connections with an exact explanation.

The all-projects overview is a workspace control panel. Its bounded **What's happening right now** graph includes only verified running or waiting sessions and PRs whose last recorded state is open. Each project lane flows from project to branch to PR. Live branches keep the coding-tool icon, exact `develop`/`main` status, and direct map navigation; GitHub-only PRs keep their published head branch and open directly on GitHub. A live branch and its PR share one row, while a PR observed through multiple local clones is counted once and repeated relations use a compact reference.

The graph shows four rows per project initially, expands to at most twelve, and pages five projects at a time. Type and text filters narrow the complete current-work index. Saved or partial GitHub evidence stays visibly labeled. The full project-card directory is collapsed below the graph so a large monitored workspace opens on current work instead of a wall of repositories.
