...
Native pairing, metadata preview, and disconnection have also been verified with fictional repositories and an isolated Electron app. Before calling team mode complete: implement local publication/recovery, GitHub installation and organization ingestion, company alerts, actual registered-app sign-in, two real member devices, offline/unshare/revoke UI verification with published work, load testing, deployment upgrades, and native accessibility. See the full [team acceptance criteria](TEAM_WORKSPACES.md) and [release roadmap](ROADMAP.md).
