...
export const attentionItem = attentionEvidenceItem.extend({
  projectId: teamId,
...
export const attentionPage = z.strictObject({
  ...
  sources: z.number().int().nonnegative().max(500),
  pendingSources: z.number().int().nonnegative().max(500),
  failedSources: z.number().int().nonnegative().max(500),
  items: z.array(attentionItem).max(100),
  omitted: z.number().int().nonnegative().max(2_650_000),
});
export const attentionDecisionCommand = z
  .strictObject({
    choice: z.enum(['snoozed', 'dismissed', 'restore']),
    items: z
      .array(z.strictObject({ id: digest, revision: digest }))
      .min(1)
      .max(100),
  })
