...
function matchTask(
  ...
  return {
    id: task.id,
    title: task.name?.trim() || 'Untitled Codex task',
    status: 'unknown', // A separate app-server cannot observe another client's live turn.
...
export function linkRepository(
  ...
  const byBranch = new Map<string, CodexTask[]>();
  const byCommit = new Map<string, CodexTask[]>();
